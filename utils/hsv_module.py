...
#
# # 각도(deg, 0~360) 기준 밴드 (현장 조명에 맞춰 ±5~10 튜닝)
# BANDS = {
#     "RED":    [(0, 20), (340, 360)],
#     "YELLOW": [(52, 82)],       # 상한 살짝 ↑
#     "GREEN":  [(82, 150)],
...
#
# def hsv_color_name(bgr_crop):
#     if bgr_crop is None or bgr_crop.size == 0:
#         return "Unknown"
#
#     # 1) 테두리 축소 + 가벼운 블러(배경/노이즈 억제)
#     h, w = bgr_crop.shape[:2]
#     bgr = bgr_crop[6:h-6, 6:w-6] if h > 12 and w > 12 else bgr_crop
#     bgr = cv2.GaussianBlur(bgr, (3, 3), 0)
#
#     # 2) HSV + S/V 마스크(색이 뚜렷한 픽셀만)
...
#         return "Unknown"
#
#     # 3) Hue(0~179) -> degree(0~360) 후 히스토그램 피크로 대표 Hue 선택
#     h_deg = (H[mask].astype(np.float32) * 2.0)
#     hist, bins = np.histogram(h_deg, bins=72, range=(0, 360))  # 5° 간격
#     peak_deg = (bins[np.argmax(hist)] + bins[np.argmax(hist)+1]) / 2
#
#     # 빨강 랩어라운드 보조 신호(0~20, 340~360 비율)
#     red_ratio = np.mean((h_deg <= 20) | (h_deg >= 340))
#
#     # 4) 밴드 매핑(+빨강 보정)
#     for name, ranges in BANDS.items():
#         if _in_band(peak_deg, ranges):
#             if name == "YELLOW" and red_ratio > 0.25:
#                 return "RED"
#             return name
#     return "Unknown"
#
# # run_project.py가 import하는 이름
# def get_color(image_array):
#     return hsv_color_name(image_array)
...
def get_color(bgr_crop):
    ...
    # 3) 경계/반사 제거: 침식으로 "핵심"만 남기기
    k = max(1, int(min(hsv.shape[0], hsv.shape[1]) * 0.015))  # 이미지 크기 비례
    ...
    h_deg = (H[mask].astype(np.float32) * 2.0)
    if h_deg.size < 30:  # 유효 픽셀 너무 적으면 보수적으로
    ...
    if _in_band(main_deg, BANDS["RED"]):
        # 붉은 표 비율이 확실히 높을 때만 RED (보강: 원본 h_deg에서 카운트)
        red_ratio = np.mean((h_deg <= 20) | (h_deg >= 340))
        if red_ratio >= 0.55:   # 임계: 필요시 0.5~0.6 사이에서 튜닝
            return "RED"
        # 그렇지 않다면 주변 밴드로 재판정
