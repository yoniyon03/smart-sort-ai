...
def process_image(image_path, conf_threshold=0.7):
    ...
    for box in result.boxes:
        ...
        if class_name == 'marker_text':
            ...
            try:
                ...
                print(f"[WARN] Failed to convert cropped image to grayscale: {e}")
                continue

            temp_crop_path = os.path.join(PROJECT_ROOT, "temp_ocr_image.jpg")
            cv2.imwrite(temp_crop_path, gray_cropped_img)
    ...
    # ⚠️ 'annotated_image'는 반환값이 필요 없어져서 None으로 변경
    # return result.plot(), final_data
    return True, final_data  # <-- (True로 수정)


# --- ⚠️ 메인 실행 (렉 걸리는 부분 주석 처리) ---
        ...
        while True:
            ...
            if new_files:
                ...
                for image_path in new_files:
                    ...
                    # ⚠️ annotated_image -> success_flag 로 변수명 변경
                    success_flag, extracted_data = process_image(image_path, conf_threshold=0.7)

                    # ⚠️ success_flag가 True일 때 (즉, 에러 없이 처리됐을 때)
                    if success_flag:
                        print(f"\n--- Final Data for {os.path.basename(image_path)} ---")
                        print(extracted_data)

                        # --- (수정) 렉 걸리는 창 띄우기 코드 주석 처리 ---
                        # cv2.imshow("YOLO Inference Result", annotated_image)
                        # cv2.waitKey(1000)
                        # ----------------------------------------
                    else:
